...
No server restart or replacement is required merely because static artifacts changed. The host reads `index.html` and unhashed static assets on each request, while content-hashed `/assets/` files and rev-addressed client bundles use immutable caching; an HMR rebuild changes the bundle rev before the browser imports new bytes. Rebuilding the relevant shell and plugin bundles, then refreshing the existing URL, is therefore the acceptance path. Starting a separate server proves only that a separate server works. If the user explicitly requests another long-running server, the existing managed background-job contract owns its lifecycle and completion notices; shell `&` is not an alternative lifecycle.
