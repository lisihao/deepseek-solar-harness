...
准入启发式仍通过 prompt 面向主模型，不是训练得到的调度 oracle。如果模型不调用 orchestration，复杂请求仍可能留在主轮次；用户也仍可强制直接策略或优先产品策略。远程 worker、Graph 间公平性、成本感知排序与动态学习并行上限不属于本决策。

## 验证

路由测试固定智能协作 TaskGraph 准入与四个可见策略标签。Scheduler 测试固定无冲突并行派发、scope 冲突等待、每 Attempt 新 lane、clean-task Capsule 注入，以及无阶段 barrier 完成。Resident Store 测试固定 lane 间并发、lane 内 single-flight 和 schema v3 历史迁移到 `legacy`。Desktop 测试固定可见 Trace 内的准入、worker、Capsule、算子与 lane 细节。
