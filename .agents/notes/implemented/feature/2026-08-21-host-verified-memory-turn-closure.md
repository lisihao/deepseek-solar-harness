...
Memory plugin tests pin real user-message detection for logs without `turn/start.trigger`, durable review counts, duplicate settled delivery, Host fallback writes, receipt projection, and restart recovery. Desktop tests pin the exact first-request pair and the promoted memory surface. Vendored-input verification maps the sealed archive to managed source and compares its tracked package contents.
