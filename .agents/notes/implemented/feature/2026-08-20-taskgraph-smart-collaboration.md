...
Autonomous Mode remains a separate, optional host policy around one RLM node. Its immutable attempt policy is sealed before the first physical Receipt, its durable state is written by `dsh-orchestratord`, and every continuation reuses the existing RLM lane instead of creating another Graph or Scheduler. Host quality gates run before continuation/turn/token/time limits; a passing gate may complete the node, while exhausted limits and retry budgets remain failures. The policy is disabled by default and is independent from a model-created RLM Goal.
...
Routing tests pin Smart Collaboration TaskGraph admission and the four visible policy labels. Allocation tests compare Codex Sol against provider-neutral high-tier planning and Luna-first against balanced execution, including the rationale retained in the sealed plan. Harness tests prove one global entry is visible from two repositories while workspace entries remain isolated. Scheduler tests pin parallel non-conflicting dispatch, scope-conflict waiting, fresh per-attempt lanes, clean-task Capsule injection, and completion without a phase barrier. Resident store tests pin concurrent lanes, single-flight within a lane, and schema-v3 history migration into `legacy`. Desktop tests pin admission, worker, Capsule, operator, and lane details in the visible Trace. The keyless Autonomous runnable snapshot pins policy resolution, root usage accounting, host-gate settlement, same-lane reuse, and the rule that limit exhaustion cannot pass a TaskGraph node. Process tests additionally pin credential scrubbing and awaited process-tree teardown on cancellation.
