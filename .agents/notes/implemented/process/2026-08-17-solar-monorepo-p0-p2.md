...
MacBook owns active DSH development and verification. Remote source contributions enter a local isolated worktree as identified commits or hash-verified patches; accepted composition evidence is retained before local changes are integrated. Mac mini deployment trees and failed execution worktrees are not source authorities. This keeps continued development independent of remote login and scheduling availability while preserving remote contributions for review.
...
The managed-source registry covers governance, Agent Teams, the controlled Desktop plugin suite, Web Billing, Web UI, and Plugin Console. Each accepted source history is imported by subtree, and [`plugins/registry.yaml`](../../../../plugins/registry.yaml) records package identity, source and upstream URLs, branch, accepted SHA, license evidence, and native checks. Exact revisions remain registry data rather than duplicated prose. The [controlled Desktop composition](../architecture/2026-08-22-controlled-desktop-plugin-composition.md) owns the current Mnemon, native vision, Aegis, and Better Sidebar decisions; Luna Vision Bridge and Memory Evolve are not managed product sources.
