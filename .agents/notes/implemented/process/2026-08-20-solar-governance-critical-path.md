...
The DSH and vendor release workflows also packed their complete npm families on every pull request. A change confined to repository documentation therefore installed dependencies, rebuilt publishable output, packed tarballs, and verified their installation even though no package input changed. Documentation-only pull request 58 measured 5 minutes 27 seconds for the DSH pack and 2 minutes 7 seconds for the vendor pack, or 9 hosted runner-minutes when each job is rounded up independently.
...
The DSH and vendor release workflows classify pull-request paths before dependency installation. Only an explicit documentation allowlist or known DSH-only input can bypass an unaffected release lane; an empty change, unknown path, package README, or vendor input runs its complete release closure. Classification uncertainty falls back to both lanes, while failure to emit a decision blocks the workflow. Push and manual runs always pack. The existing `Pack npm tarballs` jobs remain present and report an intentional bypass, so a stable required-check name does not disappear when expensive steps are unnecessary.
...
**Filter release workflows with top-level `paths-ignore`.** Rejected because an ignored pull request would create no release job, so a required check could remain absent or pending. The in-job classifier preserves a visible verdict and fails closed for paths outside the documentation allowlist.

## Consequences

Independent ordinary gates use a two-slot outer budget, while `related-tests` is the measured exclusive resource gate and native-watcher and lazy-grammar tests remain serialized inside Vitest. Gate output is emitted after each task completes, so concurrent logs remain readable. Standard runner lanes may execute longer than organization-specific larger runners, but they start without external runner provisioning and keep the public repository reproducible. A branch push without a pull request no longer receives the fork adapter's automatic verdict; opening or updating a pull request supplies the required authority, and the diagnostic matrix can still be dispatched manually. Documentation-only pull requests still pay for checkout, Node setup, and classification in each release workflow, but avoid dependency installation, builds, tarball construction, and install verification.

## Verification

Contract tests pin the shared build dependency, prepared consumer commands, exact exclusive related-test command, worker budgets, Windows Unix-socket exclusions, partial checkout, caches, and absence of a second workflow-level source build. Release-impact tests cover the documentation allowlist, package and vendor closures, empty and unknown changes, deterministic output, and workflow wiring that keeps heavy steps behind the classifier while preserving push and manual packing. Governance runtime tests cover invalid and cyclic dependencies, transitive selection, bounded independent execution, exclusive execution, and dependency failure. Acceptance requires the full strict audit, monorepo verifier, complete Code-as-Harness verification and attestation, followed by the remote pull-request CI verdict for the exact commit.

Operational measurement uses the first post-merge documentation-only pull request as a live probe. Both release jobs must remain visible and succeed, report `docs-only` with both release decisions set to `false`, and omit pnpm setup, dependency installation, build, pack, and packed-install verification. GitHub job wall time and independently rounded hosted runner-minutes are compared with documentation-only pull request 58; the required governance verdict is reported separately because this classifier does not optimize that lane.
