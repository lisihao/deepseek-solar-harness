...
DSH 与 vendor 发布工作流还会在每个 PR 上打包完整的 npm 包族。即使变更仅限仓库文档，也会安装依赖、重新构建可发布输出、打包 tarball 并验证安装，而包输入并未变化。纯文档 PR 58 的实测结果是 DSH 打包 5 分 27 秒、vendor 打包 2 分 7 秒；按每个 job 分别向上取整后共占用 9 个托管 runner 分钟。
...
DSH 与 vendor 发布工作流在安装依赖前先对 PR 路径分类。只有显式文档白名单或已知的 DSH-only 输入可以绕过不受影响的发布通道；空变更、未知路径、包 README 或 vendor 输入都会运行对应的完整发布闭包。分类不确定时回退到两条通道，无法输出分类结论时则阻断流水线。push 与手工运行始终执行打包。既有 `Pack npm tarballs` job 保持存在，并报告有意的绕过，因此不需要昂贵步骤时，稳定的必需检查名称也不会消失。
...
**使用顶层 `paths-ignore` 过滤发布工作流。** 不采用，因为被忽略的 PR 不会创建发布 job，必需检查可能因此缺失或一直 pending。job 内分类器保留可见结论，并把文档白名单以外的路径按完整发布输入处理。

## Consequences

普通独立门禁使用两个外层并发槽位，`related-tests` 是基于实测确定的独占资源门禁，原生 watcher 与懒加载语法测试仍在 Vitest 内保持串行。每个任务完成后再输出其日志，使并发日志仍可阅读。标准 runner 通道可能比组织专用的大型 runner 执行更久，但无需仓库外部配置即可开始运行，使公开仓保持可复现。没有拉取请求的分支推送不再自动获得 fork 适配器结论；创建或更新拉取请求会提供所需权威结论，诊断矩阵仍可手工调度。纯文档 PR 仍会在每个发布工作流中承担 checkout、Node 设置与分类成本，但不会再安装依赖、构建、生成 tarball 或验证安装。

## Verification

契约测试固定共享构建依赖、复用产物的消费者命令、精确且独占的 related-test 命令、worker 预算、Windows Unix-socket 排除、部分 checkout、缓存，以及工作流不再执行第二次源码构建。发布影响面测试覆盖文档白名单、包与 vendor 闭包、空变更与未知变更、确定性输出，以及把昂贵步骤置于分类器之后且保持 push 与手工打包的工作流连接。治理运行时测试覆盖无效与循环依赖、传递选择、有界独立执行、独占执行和依赖失败。验收要求依次通过严格审计、monorepo 校验器、完整 Code-as-Harness 验证与证明，并取得精确提交对应的远端拉取请求 CI 结论。

运行态测量使用合并后的第一个纯文档 PR 作为真实探针。两条 release job 必须保持可见并成功，报告 `docs-only` 且两个发布决策均为 `false`，同时不执行 pnpm 设置、依赖安装、构建、打包和已打包安装验证。GitHub job 墙钟时间及按 job 分别向上取整的托管 runner 分钟与纯文档 PR 58 对比；required governance verdict 单独报告，因为该分类器没有优化这条通道。
