...
3. `@deepseek-ai/dsh-resident-operator-local` runs an independent owner-only local-IPC daemon as the sole Session/Receipt/Lease/Event/Artifact writer. It uses a permissioned Unix socket on POSIX and a deterministic local named pipe on Windows. When a POSIX endpoint would exceed the macOS Unix-socket byte limit, shared home-path logic maps only that endpoint into a deterministic owner-specific temporary directory with `0700` parent and `0600` socket permissions; the database, receipts, and artifacts remain under the configured root. Command identity and canonical request hash are separate: identical replay returns the same receipt, changed content conflicts, crashes become `indeterminate`, and an authorized retry uses a new id with a unique link to the abandoned receipt. Forward-only table rebuilds copy each historical field by name because SQLite preserves the physical position of columns appended by `ALTER TABLE`.
4. Native product continuity remains authoritative. Claude Code uses the official Agent SDK's persisted session and resume; Codex uses pinned app-server schemas with non-ephemeral thread start/resume. Both fail closed unless current CLI/version/protocol checks prove native subscription authentication, and both receive a credential-scrubbed environment with no API fallback. Native model discovery is an admission input, while subscription quota telemetry is advisory: a telemetry outage reports unknown allowance without disabling an otherwise qualified Provider. Desktop resolves current user-owned product installations before inherited or system paths so Finder and shell launches use the same Claude Code credential client, and the Claude SDK is explicitly pinned to that resolved executable for discovery and turns rather than selecting its bundled fallback.
