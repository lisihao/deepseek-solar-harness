...
3. `@deepseek-ai/dsh-resident-operator-local` 运行独立、仅属主可访问的本地 IPC daemon，作为 Session/Receipt/Lease/Event/Artifact 的唯一写者。它在 POSIX 使用受权限保护的 Unix socket，在 Windows 使用确定性的本地命名管道。当 POSIX 端点超过 macOS Unix socket 的字节限制时，共享 home-path 逻辑只把该端点确定性地映射到当前用户专用的临时目录，父目录权限为 `0700`、socket 权限为 `0600`；数据库、Receipt 与 Artifact 仍保留在配置根目录下。Command identity 与 canonical request hash 分离：相同重放返回同一 Receipt，内容改变则冲突；崩溃变为 `indeterminate`；获授权的重试使用新 ID，并唯一关联已 abandon 的旧 Receipt。只向前迁移的表重建会按列名复制每个历史字段，因为 SQLite 会保留 `ALTER TABLE` 追加列的物理位置。
4. 原生产品连续性保持权威。Claude Code 使用官方 Agent SDK 的持久 Session 与 resume；Codex 使用固定 app-server schema 的非临时 thread start/resume。两者都在当前 CLI、版本、协议与原生订阅资格无法证明时默认拒绝，且只接收凭据清理后的环境，不提供 API fallback。原生模型发现属于准入输入，订阅配额遥测仅供调度参考：遥测不可用时报告额度未知，但不会禁用其他资格均通过的 Provider。Desktop 优先解析用户目录中的当前产品安装，再考虑继承或系统路径，使 Finder 与终端启动使用同一个 Claude Code 凭据客户端；Claude SDK 的目录发现与真实回合也显式固定到该可执行文件，不再自行选择内置后备程序。
