...
两条边界保持分离：可达性由 webserver 的绑定配置（`host: 127.0.0.1 | 0.0.0.0`）控制，客户端认证与本机所有者权威由[绑定对端的回环权威](2026-08-26-peer-bound-loopback-authority.md)负责。这道栅栏仍是混淆代理防御，而不是认证层；`trustedHosts` 点名服务权威，但不认证调用方。
...
- 非回环部署会把这道权威栅栏与 Remote Auth 组合使用；仅有可信 Host 永远不会授予 API 权威。
