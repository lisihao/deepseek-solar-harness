...
Two boundaries stay deliberately separate: reachability is the webserver binding's policy (`host: 127.0.0.1 | 0.0.0.0`), while client authentication and local-owner authority are owned by [peer-bound loopback authority](2026-08-26-peer-bound-loopback-authority.md). This fence remains a confused-deputy defense, not an authentication layer; `trustedHosts` names serving authorities without authenticating their callers.
...
- A non-loopback deployment combines this authority fence with Remote Auth; a trusted Host alone never grants API authority.
