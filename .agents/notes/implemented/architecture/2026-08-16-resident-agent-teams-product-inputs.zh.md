...
Desktop 可以在相关 package 尚未进入 DSH 公共发行版之前交付该特性，但每次 sealed input 变化都必须显式更新 manifest 与 lockfile。智能自动是主动的模型策略，不是确定性分类器或独立调度权威；排队、亲和性、成本优化和多算子 DAG 路由仍属后置。产品层会 shadow 同 id 的过期用户 Anchored Standard 副本，但不会删除用户数据。本里程碑的 Resident 发布验收仅覆盖 macOS；不受支持或发生协议变化的原生产品会 fail loud，普通 ephemeral 执行仍可用。Mac mini 部署不属于本变更，后续必须通过远程方式拉取 GitHub 正式发行版。
