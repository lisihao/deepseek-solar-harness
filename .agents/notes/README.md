...
An Agent Note is required only when a change creates or revises a durable decision whose rationale cannot live completely in code and current-state documentation. This includes cross-package architecture or ownership, a public or plugin capability contract, released on-disk, wire, session, or configuration compatibility, a security, permission, or authority rule, and repository-wide build, release, governance, or testing policy. Isolated bug fixes, feature work within existing contracts, refactors, dependency updates, and documentation or test maintenance use code, tests, current documentation, and the PR instead of creating a permanent decision record. Update an existing Agent Note only when the durable decision it owns changes, not whenever an implementation path or filename changes. A proposal for substantial future work starts in `proposed/`; a decision already made starts in `implemented/`. Pick the class folder that matches the decision (see [Classification](#classification)).
