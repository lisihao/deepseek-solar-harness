...
Launcher 把 Resident 与 AgentTeams 作为产品 overlay 组合，不会把它们持久化进用户选择的 profile。Profile 已有的 bundle 不会重复加载。Physical Consumer 会投影一份按 Session 记录的 `auto | direct | codex | claude-code` 路由策略，未配置值为 `auto`。Desktop 在主聊天模型旁只显示一个“协作”入口，并通过 `/operator` 持久化变化；它不会再把算子、算子内部模型和强度伪装成三个并列 route。面板按照人的决策顺序组织：先选智能协作、仅主模型、优先 Codex 或优先 Claude Code；只有明确某个产品后，才渐进展示其原生模型与强度高级偏好。两个高级字段都缺省为按任务推荐。打开控件时立即刷新目录，并在面板可见期间缩短刷新周期，避免启动早期请求导致一分钟的禁用假状态。底层 run 约定在省略 mode 时仍缺省为 ephemeral，智能协作则要求主 Agent 对仓库、多轮和需要跨重启连续的任务优先使用 resident。在 macOS 上，仅 owner 可访问的私有 wrapper 会把用户原生 Claude Code 与 Codex 命令暴露给打包 daemon；不会转发 API credential，也不存在 API fallback。Electron 的 `ELECTRON_RUN_AS_NODE` marker 只用于 daemon bootstrap，并会在两个产品 driver 启动子进程前移除。
...
Desktop 可以在相关 package 尚未进入 DSH 公共发行版之前交付该特性，但每次 sealed input 变化都必须显式更新 manifest 与 lockfile。智能协作是主动的模型策略，不是确定性分类器或独立调度权威；排队、亲和性、成本优化和多算子 DAG 路由仍属后置。即使选择了某个产品偏好，短对话仍由主模型负责；显式点名产品或原生模型只覆盖当前一轮。产品层会 shadow 同 id 的过期用户 Anchored Standard 副本，但不会删除用户数据。本里程碑的 Resident 发布验收仅覆盖 macOS；不受支持或发生协议变化的原生产品会 fail loud，普通 ephemeral 执行仍可用。Mac mini 部署不属于本变更，后续必须通过远程方式拉取 GitHub 正式发行版。
