...
The macOS Product Server installer accepts a stable `DSH-desktop-vX.Y.Z` tag and exact 40-character commit. It clones GitHub on the target Mac mini, builds and runs the release-shaped smoke there, drains the Host and both durable daemons through owner IPC, atomically switches `current`, preserves the preceding target as `rollback`, activates a LaunchAgent, and verifies HTTP, Remote Sync 1.4, resident providers, and the read/execute/interrupt/materialize/artifact capabilities. A default install writes a one-member `cluster.json` whose execution allowlist contains the GitHub release repository; `--execution-repo` selects another single repository, while `--cluster-config` installs an operator-supplied multi-Server membership and repository catalog. A failed activation restores the previous cluster configuration before restoring and re-verifying the previous release. PID signalling is only a fenced crash fallback after executable and instance-root identity match. No MacBook artifact is copied.
