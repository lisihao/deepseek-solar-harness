...
`dsh-desktop.mode` defaults to `compatibility`. This mode creates a normal operating-system window with its native frame and loads the official Web surface from the active DSH profile. macOS suppresses the visible page title. The operating system owns native title-bar color and appearance.
...
macOS and Linux retain the upstream adaptive directory chooser. Workspace selection therefore follows the official Web surface or native chooser selected by the supported host profile.

## Advanced mode

Advanced mode is an explicitly composed desktop presentation for macOS. After all user patches have been read, the launcher disables the official `ui-layout` Loader row, keeps the official `ui-sidebar` and `ui-conversation` rows enabled, and applies the selected mode to `desktop-shell`.
...
- The macOS tray terminal exposes private `dsh`, `pnpm`, and `node` shims. Separately, the Host runtime exposes the bundled `pnpm` command on the current Electron process `PATH` for ambient compatibility and provides the managed `desktopPnpm` service; none of these commands are added to the system `PATH`, and Linux currently has no desktop terminal command.
- `desktopPnpm.run()` and `runPlugin()` avoid shell lookup for the manager process by launching exact packaged entries. A third-party plugin that calls Node `spawn('pnpm', { shell: false })` remains non-portable and should use the managed service or a shell-aware launch path.
...
- The update handoff validates the download container, not publisher identity. macOS still requires the user to replace the application from the opened DMG. Signed artifacts, publisher verification, and native upgrade testing remain release gates.
...
- `package:dir` is an unpacked smoke artifact. Installation and upgrade behavior, native notifications and terminals, and native-material appearance remain target-platform verification boundaries.
