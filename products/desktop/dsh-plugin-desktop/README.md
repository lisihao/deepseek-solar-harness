...
The MacBook Frontend normally reaches the Product Server through an owner-controlled SSH local forward. A loopback endpoint such as `http://127.0.0.1:13080` therefore uses the authenticated tunnel directly and does not request a second pairing code or Keychain credential. Direct remote HTTPS clients, including pocket/mobile clients, still use the one-time pairing challenge, an encrypted durable device credential, and short-lived access sessions. Neither path starts a local Host while the Desktop deployment role is Frontend.
