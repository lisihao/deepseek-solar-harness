...
`yarn verify:orchestration-e2e` is the installed-product acceptance for durable orchestration. It connects through the packaged daemon clients, uses only qualified Claude Code and Codex native subscriptions, rejects a cyclic Graph, runs quality/balanced/economy allocation, proves two independent DAG leaves overlap, proves conflicting scopes serialize without deadlock, seals an enabled node-local RLM plan, recalls a prior Continuous Harness outcome, queries the running Host projection, and opens the real Desktop workbench through CDP. It writes a JSON evidence artifact under `dist/acceptance/`; module mocks cannot satisfy this gate.
