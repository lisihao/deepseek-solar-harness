...
The same package also provides `dsh-product-server`, a plain-Node Host adapter for remote deployments. Desktop and Product Server are generated from one sealed product composition and therefore load the same Resident, Orchestration, AgentTeams, Billing, Remote Modules, RLM/Continuous Harness, model-allocation, memory, governance, Synapse session-map, and product UI rows. Only Host adapter rows differ: Desktop owns Electron window, tray, terminal, profile, and updater effects; Product Server owns the persistent Web endpoint, pins the browser directory picker for remote clients, and always retains the compatibility browser layout because it does not load the Electron-owned advanced layout provider. The ordinary `dsh server` command remains the upstream-compatible bare Server profile and is not the DSH Desktop product deployment.
...
Desktop seals the Solar-controlled Better Sidebar, GenUI, plugin diagnostics, model fallbacks, code graph, Mnemon, Aegis skills, Synapse session map, and bounded utility tools as product inputs. Synapse projects existing DSH sessions and branches onto a visual canvas while DSH Session Log remains the canonical record; its removable private store contains only the plugin projection and layout state. Product-first resolution keeps the accepted Better Sidebar implementation authoritative even when an aggregate UI package depends on an older release, while its mount guard prevents duplicate sidebar ownership. Mnemon is the only product memory bundle; stale Memory Evolve rows are disabled without deleting user data. The native DeepSeek provider declares `deepseek-v4-flash-vision-exp` with image input, so Desktop does not load Luna Vision Bridge or Modlens. Aegis contributes skills only; Code-as-Harness remains the sole completion and admission authority.
...
`yarn package:dir` creates an unpacked directory for the current host platform. The packaged-runtime gate rejects an application archive that omits the desktop update and terminal modules, the DSH CLI bootstrap, the bundled pnpm entry, Resident/AgentTeams runtime packages, Code-as-Harness governance, the patched Anchored Standard preset, or the physical deployment package. Electron Builder emits the root manifest, desktop runtime, and complete dependency tree under `app.asar.unpacked`; both Host profile boot and the CLI bootstrap use this physical tree so DSH profile-fallback symlinks never target a virtual ASAR directory. `verify:vendor` rejects stale installed file dependencies before packaging, `verify:composition-package` composes those product inputs from the packaged Electron tree, while `verify:resident-package` is the offline-safe packaged Resident smoke: by default it resolves both native `claude` and `codex` command entries, checks Provider structure, Resident protocol/state schema, daemon startup, and cleanup, and does not require either subscription to be online or qualified. Pass `--require-provider codex` or `--require-provider claude-code` (repeat the option to require both) when environment qualification is itself the gate; every required Provider must report available native-subscription authentication or the command fails. `--execute` opts into real no-tool turns only for Providers that are available; if no native-subscription Provider is available, it fails rather than passing with zero executions. An explicitly required but unavailable Provider also fails before execution, and the smoke never silently falls back to an API key. Thus `verify:resident-execution` is the same smoke with `--execute`, while ordinary `yarn verify:resident-package` remains safe for packaging and CI without live subscriptions. Examples are `yarn verify:resident-package`, `yarn verify:resident-package --require-provider codex`, and `yarn verify:resident-execution --require-provider codex`. `build/app-icon.png` remains the unmodified iOS Default source and the Windows/Linux application icon. The build runs `scripts/generate-mac-app-icon.mjs` to center that artwork at 824 by 824 pixels on a transparent 1024 by 1024 canvas; macOS packaging and the live Dock both use the generated `build/app-icon-mac.png`. `build/tray-icon.svg` is the brand-blue tray source: the build derives a macOS template image that the system colors automatically and fixed brand-blue Windows and Linux tray images.
