...
- Desktop resolves the user-owned `~/.local/bin`, `~/.npm-global/bin`, `~/.bun/bin`, or `~/.volta/bin` product installation before an inherited system PATH. This keeps Finder and terminal launches on the same qualified Claude/Codex release and avoids a root-owned legacy Claude CLI sharing and rotating the same macOS Keychain credential.
