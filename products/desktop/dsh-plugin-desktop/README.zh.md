...
- Desktop 会先解析用户目录中的 `~/.local/bin`、`~/.npm-global/bin`、`~/.bun/bin` 或 `~/.volta/bin` 产品安装，再考虑继承的系统 `PATH`。这样 Finder 与终端启动会使用同一份已通过资格审查的 Claude/Codex 版本，并避免旧的 root-owned Claude CLI 共享和轮换同一个 macOS Keychain 凭据。
