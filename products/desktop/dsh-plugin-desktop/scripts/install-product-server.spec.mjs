...
test('requires a stable Desktop tag pinned to one exact commit', () => {
  const options = parseInstallerArguments([
    '--ref', 'DSH-desktop-v3.9.6', '--commit', commit, '--port', '13080', '--host', '127.0.0.1',
  ])
  assert.equal(options.ref, 'DSH-desktop-v3.9.6')
  ...
  assert.throws(() => parseInstallerArguments(['--ref', 'DSH-desktop-v3.9.6', '--commit', 'short']), /40-character/)
})
