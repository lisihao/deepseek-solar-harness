...
`web`、`server` 和 `headless` profile 首次使用时会从随附模板自动初始化。`web` 挂载 base + web-app；`server` 在这一浏览器表层上增加 Resident 物理算子和持久 TaskGraph 编排组合包；`headless` 挂载 base + headless。其他缺失的 profile 会显式报错，并提示运行 `dsh plugin --profile <name> add <package>`。
...
| `server` | 同上 Web 参数；启动器还会固定加入 `--deployment-role server` |
