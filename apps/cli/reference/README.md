...
Bundle names resolve from the dsh installation first, then from the profile directory. In-box bundles (`@deepseek-ai/dsh-base`, `@deepseek-ai/dsh-web-app`, `@deepseek-ai/dsh-headless`, `@deepseek-ai/dsh-resident-operators`, `@deepseek-ai/dsh-orchestrations`) therefore always come from the same installation as the running `dsh`; out-of-tree bundles come from the profile's pnpm-managed `node_modules`. A bare plugin `name` in any patch row resolves through the profile directory's Node parent-walk, which reaches the maintained installation fallback `$DSH_HOME/profiles/node_modules` (one symlink per package the installation's app and bundles depend on, healed on every launch).

The `web`, `server`, and `headless` profiles auto-initialize from shipped templates on first use. `web` mounts base + web-app, `server` adds the Resident Physical Operator and persistent TaskGraph orchestration bundles over that browser surface, and `headless` mounts base + headless. Any other missing profile fails loud with a hint to run `dsh plugin --profile <name> add <package>`.
...
| `server` | the Web arguments above; the launcher also fixes `--deployment-role server` |
