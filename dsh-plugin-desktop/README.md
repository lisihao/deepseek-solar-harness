...
The product layer adds AgentTeams and the existing physical-operator tool surface. Resident execution is opt-in and returns bounded continuity metadata; the default request remains ephemeral. The packaged Anchored Standard preset keeps both top-level and delegated agents on the two-tool first-turn bootstrap. AgentTeams puts its coordination protocol in the worker's first user message and preserves the preset persona.
