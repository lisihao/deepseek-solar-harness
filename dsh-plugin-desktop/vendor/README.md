...
- Resident Physical Operator tarballs are built from `lisihao/deepseek-solar-harness` commit `7b87464619b4527cd77cb6cc925406db17fdcdb4`. This revision qualifies subscription authentication per execution mode, uses the macOS system CA store for Claude Code subscription traffic, classifies expired OAuth explicitly, carries a sanitized display-only task label into durable receipts, preserves deterministic Resident routing and keeps client disconnects from interrupting native turns.
