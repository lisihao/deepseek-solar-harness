...
- **Governed related tests** cap Vitest at four workers. The branch-wide `--changed=origin/master` inventory includes process- and socket-heavy SDK suites; bounding aggregate fork pressure keeps their local HTTP deadlines deterministic instead of turning host CPU contention into false regressions. Focused developer runs may still choose a narrower worker count.
