...
- **Governed related tests** cap branch-wide `--changed=origin/master` runs at four workers. This keeps process- and socket-heavy SDK suites within local HTTP deadlines under fork contention; focused runs may use fewer workers.
- **Isolated managed workspaces** own deterministic install gates in the Code-as-Harness DAG. Their build, typecheck, test, and documentation gates must declare that install gate through `needs`; a fresh worktree must never pass only because another task left `node_modules` behind.
