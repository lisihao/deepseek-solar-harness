...
- **受治理的关联测试**把分支级 `--changed=origin/master` 运行限制为四个 worker，使进程与 socket 密集的 SDK 测试在 fork 争用下仍满足本地 HTTP 截止时间；聚焦运行可使用更少 worker。
- **隔离的受管工作区**在 Code-as-Harness DAG 中拥有确定性的安装门禁。其构建、类型检查、测试和文档门禁必须通过 `needs` 声明该安装门禁；fresh worktree 绝不能仅因为另一个任务遗留了 `node_modules` 而通过。
