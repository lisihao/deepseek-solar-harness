...
- **受治理的关联测试**把 Vitest 上限设为四个 worker。分支级 `--changed=origin/master` 清单包含进程与 socket 密集的 SDK 测试；限制聚合 fork 压力可让本地 HTTP 截止时间保持确定，避免把宿主 CPU 争用误判为回归。开发者的聚焦运行仍可选择更窄的 worker 数。
