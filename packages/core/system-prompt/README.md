...
- `captureRuntimeContextSnapshot(messages, sourceSessionId)` — system-prompt owns the canonical detached v1 projection for downstream request records. It carries the source Session and snapshot-message identities with named sections; a clearance marker returns `undefined` rather than reviving earlier context.
...
- **No escape syntax inside an interpolated section** — every complete `{{…}}` group is resolved against registered variables. A generated section that documents another template language opts out with `interpolate: false`; mixed literal and DSH-variable groups in one section are not supported.
