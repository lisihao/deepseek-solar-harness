...
Local implementation of the [`dsh-sandbox`](../sandbox/) seam. It selects and caches one supported platform runner: Linux prefers a working `bwrap` then Landlock; macOS uses Seatbelt. Multiple Linux candidates are probed in order, while macOS's sole candidate is selected directly.
