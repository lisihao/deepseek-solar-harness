...
- **冷列表修复采用 fail-soft**：日志未变化且投影绑定 revision 时精确并保持零 I/O；记录缺失或不匹配时执行一次锚定尾部折叠并缓存观测到的 revision。读取或缓存写回失败仍只会降级到保持可见、排序偏旧，不会隐藏会话。精确投影刷新不可用时，大于 `coldBlankProbeMaxBytes` 的空白工件或来自不提供 `locate()` 后端的空白工件继续保持可见。[有界空白验证决策](../../../.agents/notes/implemented/bug-fix/2026-08-13-bounded-cold-blank-verification.md)规定该安全方向；把活动时间直接存入持久化索引仍属于[最后活动索引提案](../../../.agents/notes/proposed/architecture/2026-07-29-durable-last-activity-index.md)的范围。
