...
ChatGPT 网页版作为主模型时，面板隐藏下游协作选项（纯文本路由不会使用它们），只显示 Web 模型和推理强度控件。Web 模型控件可就地读取账户目录（不发送提示），随后提供观察到的模型和推理强度。
