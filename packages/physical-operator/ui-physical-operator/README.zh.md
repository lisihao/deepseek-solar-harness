...
“刷新模型与算子”按钮请求最新 API 模型目录、绕过 Resident 资格缓存，并发现账户可见的 Web 模型与推理控件。各来源分别报告成功或失败；失败来源保留上一次成功列表。刷新不会改变所选主模型、启动认证或发送模型提示。Web 任务运行时不能刷新 Web，但不妨碍其他来源刷新。Web 模型与推理偏好使用独立且经验证的控件，不借用原生 CLI 强度标签。
