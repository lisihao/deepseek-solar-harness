...
While ChatGPT Web is the primary model, the panel hides the downstream collaboration choices, which cannot apply to a text-only route, and shows only the Web model and reasoning controls. The Web model controls can read the account catalog in place without sending a prompt, and then offer the observed models and reasoning levels.
