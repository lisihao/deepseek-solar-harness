...
When the Host reports tool coordination unavailable (`coordinatorAvailable: false`), the ChatGPT Web section offers only direct questions: it hides the coordination heading and guidance, the tool-coordination choice, connector status, MCP verification, and connection setup. While ChatGPT Web is the primary model outside tool coordination, the panel hides the downstream collaboration choices, which cannot apply to a text-only route. The Web model controls can read the account catalog in place without sending a prompt, and then offer the observed models and reasoning levels.
