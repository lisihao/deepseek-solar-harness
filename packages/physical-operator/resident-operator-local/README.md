...
Codex response-stream transport failures, including a disconnected responses request, are reported as `RUNTIME_UNAVAILABLE`; malformed terminal output remains `INVALID_RESULT`. Orchestration callers can retry only the transient class.
