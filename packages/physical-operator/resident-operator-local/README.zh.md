...
- 协议 v4 与 state schema v3 只支持本地 Unix socket，schema v1 和 v2 都会增量迁移，正式验收平台为 macOS；Windows named pipe 后置。
