...
- 协议 v10 与 state schema v5 只支持本地 Unix socket，schema v1 至 v3 会迁移到兼容 `legacy` lane，schema v4 会新增压缩 Receipt 表，正式验收平台为 macOS；Windows named pipe 后置。
