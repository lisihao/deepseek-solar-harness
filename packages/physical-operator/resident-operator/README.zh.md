...
- 协议 v9 不包含人工写接管与 control lease。
- Durable Jobs 投影与亲和调度器是独立的后续 Consumer。
- v9 面向本地 Provider；远程传输和 Windows named pipe 后置。
