...
`execute()` 接收调用方生成的持久 command ID、算子 ID、工作区、content blocks、可选的有界展示任务摘要、可选模型/强度偏好与取消信号。任务摘要不是 prompt，只用于让重连后的用户界面在不持久化原始任务内容的前提下识别工作。Provider 根据实时产品目录补全省略的 profile 字段，并把有效 profile 锁定到 Session。经显式授权的重试可以关联一条已 abandon 的 indeterminate command，但必须使用新 command ID。`list()`、`inspect()`、`inspectTurn()`、`readEvents()`、`interrupt()`、`reset()` 与 `resolveIndeterminate()` 只供可信插件和 CLI 管理消费者使用。
