...
- The response collector accepts legacy role-marked messages and current `data-content-search-unit-key` user and assistant units. It removes nested duplicate units, converts the latest `data-markdown-text-style="assistant-message"` body to Markdown (paragraphs, headings, lists, quotes, tables, fenced code without the block toolbar, KaTeX from its TeX source as inline `$…$` or as a display block with each `$$` on its own line, so multi-line TeX stays one closed block, and citations as source links), and requires two matching non-generating samples. When the page exposes the reply's React turn item, that item decides: only a `completed` item whose `phase` is absent or `final_answer` settles, and an incomplete item counts as generating even beside a copy action. Without a readable item, for a current unit, only an exact `Copy`, `复制`, `Copy response`, or `复制回复` button in its own single-assistant action ancestor, outside the answer body, can mark the response settled; code-block and table copy buttons inside a streaming answer never do, and the search stops before `main`, `body`, or a multi-assistant ancestor. A `stop-button` test id or a stop label marks the reply as still generating.
