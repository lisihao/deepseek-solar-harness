...
- The response collector accepts legacy role-marked messages and current `data-content-search-unit-key` user and assistant units. It removes nested duplicate units, extracts the latest `data-markdown-text-style="assistant-message"` body, and requires two matching non-generating samples. For a current unit, only an exact `Copy` or `复制` button in its own single-assistant action ancestor can mark the response settled; the search stops before `main`, `body`, or a multi-assistant ancestor.
