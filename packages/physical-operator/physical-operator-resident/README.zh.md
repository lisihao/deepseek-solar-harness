...
Ephemeral 分支调用 `ctx.subagents`；Resident 分支只调用 `ctx.residentOperators` Service Definition，不 import 本地 daemon Provider。因此 Provider 与 Consumer 可在 capability seam 后独立替换。
