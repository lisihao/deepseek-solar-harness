...
The same package advertises every available physical operator through the `dsh-physical-operator` model route. Codex and Claude Code can therefore be selected as first-class main models, with no DeepSeek API key or DeepSeek request in that path. The route also lists one `operator:model` entry per model in each available Codex or Claude Code live catalog, named `<operator> · <model>` with that model's native efforts. Selecting one routes to that operator with the model and effort as its Resident profile, overriding the saved `/operator-profile`. Qualifying a native product spawns its CLI, so native entries appear only after an explicit model refresh; plain directory reads list the operators and reuse the last refreshed catalog, and a refresh reports catalog failure. When its optional Provider is installed, `chatgpt-web` is also a first-class direct route through the user's authenticated ChatGPT website. The deterministic host classifier does not make it the primary route, while a coordinating model may choose an available catalog entry as a bounded advisor. Codex and Claude Code receive the exact DSH system prompt and model-visible tool schemas for the turn. Calls return through an owner-local bridge into the original Agent's `ctx.tools`, so existing scope, guard, approval, plugin ownership, and result rendering continue to apply. ChatGPT Web intentionally runs only as an ephemeral browser subscription: it receives the task, any current Web handoff or steering, and matching task-template instructions, but not the tool-using agent's system instruction or runtime context, and no synthetic DSH tool bridge, no DSH model/effort preference, and no claimed Resident continuity; its website account owns those choices. The bridge records ignorable call/result events and reconstructs tool receipts after DSH reload. Each pair retains the receipt `commandId` and stable `toolCallId`, plus the parent physical execution id used by trajectory projections. A settled call returns its recorded result; a changed request conflicts; a call observed without a result persists an explicit indeterminate trace instead of remaining visibly running or being replayed automatically. If its result arrives while the same binding remains active, the bridge refreshes from the durable authority log, caches that result, and never invokes the tool again. Raw arguments, results, errors, prompts, and Provider text remain only in the durable authority log; the Host strips them before public history or mux delivery and emits only a fixed, text-free trace schema.

When an admitted `chatgpt-web-handoff` and direct-user steering share one request, only the selected ChatGPT Web main route receives the current step's latest handoff. The final direct-user message remains the task, while earlier direct-user messages after the handoff become labeled context. Handoffs from earlier steps and every other plugin message stay outside the fresh Web lane.
...
Every Session also has a durable routing policy. Untouched Sessions project `Smart Auto`; deterministic host routing recognizes bounded implementation/debugging work as Codex-shaped and bounded analysis/research work as Claude-Code-shaped. `chatgpt-web` is deliberately absent from that automatic classifier. Complex parallelizable work remains on the primary turn so `@deepseek-ai/dsh-tool-orchestration` can construct a persistent TaskGraph. An explicitly enabled Debate mode has the same higher-order precedence: the physical router records a TaskGraph candidate and leaves the user turn to the Debate Consumer instead of dispatching one Resident. A Codex or Claude Code preference carries into TaskGraph execution as per-node `preferredIds`; bounded standard work still dispatches one Resident directly. `/operator codex`, `/operator claude-code`, `/operator chatgpt-web`, `/operator direct`, and `/operator auto` provide visible manual overrides. `/operator-profile <product> <model|auto> <effort|auto>` applies only to Codex and Claude Code; ChatGPT Web intentionally exposes no DSH-side model or effort control. Non-reset model/effort pairs are checked against the live Resident catalog before the profile event is logged; an unavailable catalog rejects the change without mutating Session state, while `auto/auto` remains available to clear a stale preference. The daemon repeats the check at execution time because catalogs can change. When a session-local model selection is installed and captured for a step, the selected API or physical model remains the coordinator. A selected physical model is host-routed directly; a selected API stays on its API route and receives the logged policy as downstream collaboration and TaskGraph guidance, except that Smart Auto may route one recognized request to a physical operator or ask the API coordinator to start a TaskGraph. Current-message mentions of Codex, Claude Code, or ChatGPT Web and saved `/operator` policies do not replace it. A selected native Codex or Claude Code main model retains the real DSH tool bridge and may call downstream collaborators, while a preference names only a collaborator. A selected ChatGPT Web main model in direct mode remains its ephemeral browser route without a synthetic bridge or DSH profile. Smart Collaboration may choose an available ChatGPT Web catalog entry only as a bounded advisor for design, high-level planning, or research; that catalog-advisor route has no DSH file-write or test-execution capability and cannot establish acceptance. Without an installed model selection, legacy host routing continues: outside an explicitly enabled higher-order mode, a product or recognizable native model family named in the current message wins over the stored preference (`Sonnet`/`Opus`/`Haiku` select Claude Code; `GPT-5.x` selects Codex; explicit `ChatGPT Web` selects `chatgpt-web`). An accepted direct route replaces only that model step with a physical-operator adapter and logs the displaced primary model config beside the dispatch. Only Resident routes use historical routing state to reconnect an undelivered command receipt or continue a prior task; ephemeral browser routes are never automatically re-run from a historical continuation or recovery record. Routing decisions, dispatches, policies, and profiles are durable and ignorable by older readers. Smart Auto retries only an unadmitted selected operator reporting `AUTH_MODE_MISMATCH`, `OPERATOR_UNAVAILABLE`, `PROVIDER_VERSION_MISMATCH`, or `RUNTIME_UNAVAILABLE`; malformed qualification output, quota exhaustion, and product-command failures remain errors from the selected route. `/operator direct` stays on its route without an unrelated Claude probe or a DeepSeek API fallback.
