...
- **Extension and signature media type** — a supported extension declares the type and a missing extension uses the PNG/JPEG/WebP/GIF signature; the attachment store's full decode remains authoritative. A correctly formatted image under a wrong extension is still refused with the rename remedy rather than sniffed.
