...
An explicit `ctx.llm.listModels('deepseek-official', { refresh: true })` reads `GET /models` at the resolved `baseURL`, using the same resolved credential, app attribution, and anonymous request id as a provider request. It never starts a chat completion or inference. Omitted options and `{ refresh: false }` read the current catalog without network I/O. The live cache is scoped to the resolved endpoint and credential reference; a late refresh cannot replace a newer result for that scope. The configured `streamIdleTimeoutMs` also bounds the complete catalog fetch and JSON body read, with timeout reported as `TIMEOUT`. A successful refresh replaces cached live ids; configured entries retain their own name, description, and modalities, while a newly reported id gets only the existing text-only selector fallback, with no inferred capacity, output limit, or rate capability. Invalid, empty, and duplicate wire ids are discarded. A failed refresh rejects with its provider error while the next ordinary read retains the last successful catalog; a missing credential rejects with `MISSING_CREDENTIAL` before network I/O.
