...
Resident 物理算子执行会按 `commandId` 聚合为独立组，展示派发、阶段、公开输出摘要、工具生命周期标签、严格结构化的工具参数／结果摘要、审批请求、用量更新及终态／降级标记。结构摘要只允许一组封闭的状态／计数字段公开标量值；任意字符串、prompt 内容、凭据、推理、工具输出和终端文本始终隐藏。桥接工具调用和结果按稳定的 `toolCallId` 配对；恢复时只有调用而没有可证明结果的 Receipt 会显示为 `indeterminate`，不会永久显示为运行中，也绝不自动重放。绝不投影 prompt、thinking、stderr、环境、凭据或与最终助手回复重复的完整文本。
