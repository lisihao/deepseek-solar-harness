...
Resident 物理算子执行会按 Host 伪名聚合为独立组，展示派发、封闭的阶段／状态词汇、无文本的公开输出和工具生命周期标记、值结构形状、用量计数及终态／降级标记。Host 会在 history 或 mux 交付前清空每个 `physical-operator/*` 事件 payload，并附加固定 schema 的安全轨迹；其中不含 Provider 字符串、属性名、prompt 内容、凭据、推理、工具输出、错误或 transcript。UI 只消费这份安全轨迹，因此实时事件与旧 Session replay 受同一边界约束。桥接工具调用和结果按稳定的 Host 伪名配对；只有调用而没有可证明结果的 Receipt 显示为 `indeterminate`，后来到达的持久结果会结算既有行且绝不重放工具。
