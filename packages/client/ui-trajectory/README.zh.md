...
Resident 物理算子执行会按 `commandId` 聚合为独立组，展示派发、阶段、公开输出摘要、工具生命周期标签、经过清洗且有界的工具参数／结果摘要、审批请求、用量更新及终态／降级标记。桥接工具调用和结果按稳定的 `toolCallId` 配对；绝不投影 prompt、thinking、stderr、环境、凭据或与最终助手回复重复的完整文本。
