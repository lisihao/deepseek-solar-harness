...
Node 侧会扫描已启用的 Loader 配置项以发现 web `dsh.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并通过 `/plugins` 提供该文件及其 sourcemap。携带当前 `rev` 的 bundle URL 会按 `Accept-Encoding` 使用 Brotli 或 gzip，并采用不可变缓存策略；缺失或过期的修订参数与 sourcemap 仍为 `no-cache`。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。
