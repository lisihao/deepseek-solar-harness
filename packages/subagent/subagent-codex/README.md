...
The production wire intentionally implements only the app-server methods required by this one-shot contract. Development evidence is pinned to `@openai/codex@0.149.1` / `codex-cli 0.149.1`; the npm package is a test-only dependency, and deployments still supply `codex` on `PATH`.
