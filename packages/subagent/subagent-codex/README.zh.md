...
对于命令与文件审批，无人值守的提供方会从请求给出的决策选项中选择一项不予批准的决策，并优先选择 `cancel`；稳定的 0.151.0 请求形态没有决策选项列表，因此回退到 `decline`。它对权限请求返回作用域限于当前轮次的空权限集，不向用户输入请求提供任何答案，并拒绝 MCP elicitation。若请求在无人值守模式下没有合法响应，或是未知服务器请求，此次运行就会失败。
...
当 `env` 为空时，Provider 声明 `authentication.mode: native-subscription`；只要存在任一显式条目，它就声明 `explicit-environment`。这是有意采取的保守规则：即使覆盖项不含秘密，Provider 也无法再声明执行路径完全由原生账户状态决定。物理算子适配器只接受处于 `native-subscription` 模式的 `codex`，对显式声明或缺失声明都会按默认拒绝处理。若部署方有意选择 API 计费，通用 subagent Provider 仍可使用显式环境。
...
生产环境的协议层有意只实现这一单次执行约定所需的 app-server 方法。开发证据锁定在 `@openai/codex@0.151.0` / `codex-cli 0.151.0`；该 NPM 包仅作为测试依赖，部署环境仍需通过 `PATH` 提供 `codex`。
...
- **声明由配置推导**：`native-subscription` 只能证明 DSH 没有提供显式子进程环境；仍须用真实 canary 证明宿主当前原生登录有效且由订阅套餐承载。
- **兼容性由开发证据锁定**：若要从已验证的 0.151.0 协议基线升级，必须重新生成上游 schema 证据，并重新运行握手、答案选择、审批、取消、无密钥真实产品以及带密钥的 DeepSeek 随机数测试。
