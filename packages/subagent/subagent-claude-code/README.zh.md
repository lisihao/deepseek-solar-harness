...
当 `env` 为空时，Provider 声明 `authentication.mode: native-subscription`；只要存在任一显式条目，它就声明 `explicit-environment`。这是有意采取的保守规则：即使覆盖项不含秘密，Provider 也无法再声明执行路径完全由原生账户状态决定。物理算子适配器只接受处于 `native-subscription` 模式的 `claude-code`，对显式声明或缺失声明都会按默认拒绝处理。若部署方有意选择 API 计费，通用 subagent Provider 仍可使用显式环境。
