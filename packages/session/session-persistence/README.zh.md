...
| `replicate(replica, signal?): Promise<SessionReplicationResult>` | 仅当目标不存在或是来源的精确前缀时，应用一份完整、平衡的冷日志。重放返回 unchanged，目标更长时仍以目标为权威；目标处于活动状态、来源轮次未结束、元数据不一致或事件历史分叉时，以 `SessionReplicationError` 拒绝。 |
...
- **副本写入保持单一权威。**`replicate` 在本 Service 内串行化同 id 调用，并且只推进精确前缀。部署交接仍必须先让旧写者停稳，再激活目标；该方法不建立分布式锁，也不合并分叉日志。
...
- **复制传输稳定边界，而不是活动轮次**：实时进展继续通过观察流提供；只有完整、平衡的日志才能成为目标端可恢复副本。
