...
- Remote members currently execute read-only/result-producing nodes against an already available Server workspace. Cross-machine Git worktree synchronization, cluster Leader election, and failover of the TaskGraph authority are separate layers and are not implied by remote Provider discovery.
