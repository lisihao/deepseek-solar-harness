...
For Resident RLM, the sealed ExecutionPlan contains both the low-tier worker allocation and the high-tier synthesis allocation. The daemon executes bounded branch levels on fresh native lanes, persists every branch as a content-addressed artifact, and then performs one synthesis turn. `maxDepth`, `maxChildren`, `maxTurns`, Graph parallelism, and provider capacity are mechanically enforced. The composite occupies one global Scheduler slot, so node-local recursion cannot become a competing TaskGraph or oversubscribe concurrent DAG work.
