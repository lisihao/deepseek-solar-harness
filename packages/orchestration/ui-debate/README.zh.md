...
不带 `run_id` 的 GET 返回有界的持久化 Run 列表；带 `run_id` 的 GET 还返回所选 Run、角色与轮次状态、Claim Ledger、保留异议、未决项、usage/费用归集状态、综合结果 Artifact 引用，以及有界的游标事件页。POST 只在携带专用控制头并通过本地回环或已配对远程设备认证时，接受带 revision 栅栏的 `approve`、`reject`、`pause`、`resume` 或 `stop` 控制。
...
- UI 展示有界预览和 Artifact 引用，不展示完整模型 transcript 或大型 synthesis 文档。
