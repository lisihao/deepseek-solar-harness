...
GET without `run_id` lists bounded persistent runs. GET with `run_id` adds the selected run, role and round state, claim ledger, preserved dissent, unresolved items, usage/cost accounting status, synthesis Artifact reference, and a bounded cursor event page. POST accepts revision-fenced `approve`, `reject`, `pause`, `resume`, or `stop` controls only with the dedicated control header and an authorized loopback or paired remote-device identity.
...
- The UI presents bounded previews and Artifact references, not complete model transcripts or large synthesis documents.
