...
GET without `run_id` lists bounded persistent runs. GET with `run_id` adds the selected run, public roster responsibilities, every round's bounded per-agent output summaries and Artifact references, claim ledger, preserved dissent, unresolved items, usage/cost accounting status, initial plan, durable continuation state, moderator synthesis, and a bounded cursor event page. POST accepts revision-fenced `approve`, `reject`, `pause`, `resume`, `stop`, or eligible `continue` controls only with the dedicated control header and an authorized loopback or paired remote-device identity.
...
The browser panel uses the shared theme tokens but owns its styles and transport. It shows the selected initial plan and reason, distinguishes convergence, a round limit, and token or cost exhaustion, and leaves every caller-provided monetary cap intact. This package depends only on the provider-neutral `ctx.debates` Service Definition and Host/client platform seams; it does not import the local Debate Provider, orchestration daemon, or physical-operator runtime.
...
- Durable public output previews and moderator synthesis render through the shared safe Markdown renderer. Headings, lists, quotes, tables, and priority markers remain structured; raw HTML is never executed or displayed as a layout primitive. When a continuation begins, the current moderator synthesis is pending and the sealed earlier synthesis remains available in history.
- Friendly route, fallback, artifact, usage, and timing data remain available under collapsed execution details. Unresolved claims, preserved dissent, and separate Run, Round, and convergence states keep a budget limit from being presented as a contradictory active or stopped state. An eligible settled `completed`, `max_rounds`, or `budget_limited` Run has one visible `继续讨论 2 轮` control with its next-round and token allowance. The control sends a fresh command id plus the displayed revision, disables while pending, and refreshes the durable projection before another deliberate extension. Unknown accounting, an unsettled final round, and a cost cap requiring the existing approval path show an explanation instead of a continuation control.
