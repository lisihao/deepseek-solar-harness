...
受管条目通过乐观并发控制作用于会话本地、工作区或用户全局作用域。全局作用域在所有者的 DSH home 下拥有唯一稳定身份，因此同一条目可以跨仓库可见，不必复制进任一工作区。Refinement 规划不修改生效中的 Harness；应用只在声明的 Turn 边界发生，逐项独立报告编辑结果，并仅记录成功项以供显式回滚。不可变 base prompt 不能被修改。
