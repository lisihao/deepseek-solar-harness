...
2. Later rounds use `claim-ledger`, or `high-severity-unresolved` when a high/critical unresolved gap remains. Every claim, dissent, and unresolved entry must reference a `claim_id` already in the prior ledger; an unknown follow-up ID fails that turn and cannot expand the topic.
