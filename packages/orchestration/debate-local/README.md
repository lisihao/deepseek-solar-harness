...
- After convergence evaluation reaches `converged`, `budget_limited`, or `max_rounds`, the run remains `synthesizing` until `debate.synthesis.settled` commits the final `completed`, `budget_limited`, or `max_rounds` state. A `continue` command is accepted only from one of those settled outcomes with a fully settled last round and sufficient accounting. Under the write lock it records an immutable two-round grant, the previous settled moderator summary, and a pending replacement summary; old rounds and their attempts stay sealed. Newly admitted runs persist a public topic from `objective`, or from `prompt` when no objective is supplied; legacy records without topic text remain missing rather than borrowing another topic.
- The initial policy stays immutable. Effective round, turn, and token ceilings are derived from it plus grants. A grant adds exactly two rounds, two turns per roster slot, and at least two normal round allowances or the observed capacity of the two latest complete rounds. It never changes `maxCostUsd`; a metered forecast that cannot fit the caller cap reports the cap and requires the existing approval path.
- `control` can approve, pause, resume, stop, reject, or continue. A running pause is persisted as a round-boundary intent, while stop aborts the injected round executor; an interruption whose downstream TaskGraph outcome cannot be proved becomes `indeterminate`. A paused run can be resumed only with its current revision and a matching `resume` command.
...
The immutable policy and durable grants bound agents, rounds, turns, and tokens. The local Provider adds no model prompt of its own and records only usage reported by the executor; native-subscription usage is not reported as API spend.
