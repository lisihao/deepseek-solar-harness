...
每个持久化的公开 Debate 事件还会作为一条可忽略的 `debate/trace` 会话事件写入，并以 `(runId, sourceSequence)` 去重。该事件只携带当时可获得的议题、轮次、易读角色路由、有界公开输出、主张、Evidence 引用、收敛判断或主持人综合结果。此投影不会向会话日志伪造 assistant 消息，也不会写入原始提示词、私有推理、凭据或原生产品 transcript。
