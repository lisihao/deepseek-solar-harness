...
All commands accept `--json`. `verify` also accepts `--dry-run` and `--fail-fast`. Profiles contain argument arrays rather than shell strings, so the runner does not evaluate shell syntax. A profile may set a positive `max_concurrency` and declare gate `needs`; verification runs only dependency-ready gates, includes dependencies transitively in the plan, and blocks consumers when a prerequisite fails.
