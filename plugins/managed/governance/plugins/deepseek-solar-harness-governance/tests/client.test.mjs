...
test('trace source distinguishes rejected and invalidated work from unmanaged sessions', async () => {
  ...
  assert.match(source, /查看完整 Evidence 输出/u)
  assert.match(source, /evidence_ref/u)
  assert.match(source, /block\.type === 'reasoning'/u)
  assert.match(source, /智能协作、首模型与 Resident 算子/u)
})
