...
EOF
    ...
    cat > "$repo/assets/aegis-small.svg" <<'EOF'
...
write_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/aegis/skills/example"
    printf 'fixture keep\n' > "$repo/plugins/aegis/.fixture-keep"
    cat > "$repo/plugins/aegis/skills/example/SKILL.md" <<'EOF'
# Example Skill

Fixture content.
EOF
    git -C "$repo" add plugins/aegis/.fixture-keep
    git -C "$repo" add plugins/aegis/skills/example/SKILL.md
...
write_synced_destination_fixture() {
    ...
    mkdir -p \
        "$repo/plugins/aegis/.codex-plugin" \
        "$repo/plugins/aegis/.private-journal" \
        "$repo/plugins/aegis/assets" \
        "$repo/plugins/aegis/skills/example"

    cat > "$repo/plugins/aegis/.codex-plugin/plugin.json" <<EOF
{
  "name": "aegis",
  "version": "$MANIFEST_VERSION"
}
EOF
    ...
    cat > "$repo/plugins/aegis/assets/aegis-small.svg" <<'EOF'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1 1"></svg>
EOF

    printf 'png fixture\n' > "$repo/plugins/aegis/assets/app-icon.png"
...
EOF
    ...
    printf 'tracked keep\n' > "$repo/plugins/aegis/.private-journal/keep.txt"

    git -C "$repo" add \
        plugins/aegis/.codex-plugin/plugin.json \
        plugins/aegis/assets/app-icon.png \
        plugins/aegis/assets/aegis-small.svg \
        plugins/aegis/skills/example/SKILL.md \
        plugins/aegis/.private-journal/keep.txt
...
write_stale_ignored_destination_fixture() {
    ...
    mkdir -p "$repo/plugins/aegis/.private-journal"
    printf 'fixture keep\n' > "$repo/plugins/aegis/.fixture-keep"
    printf 'stale ignored leak\n' > "$repo/plugins/aegis/.private-journal/leak.txt"
    git -C "$repo" add plugins/aegis/.fixture-keep
...
main() {
    ...
    dirty_skill_path="$dirty_apply_dest/plugins/aegis/skills/example/SKILL.md"
    ...
    assert_contains "$preview_section" "assets/aegis-small.svg" "Preview includes SVG asset"
    ...
    assert_not_contains "$preview_output" "Assets (aegis-small.svg, app-icon.png) will be seeded from" "Preview omits assets seeding note"
    ...
    assert_branch_absent "$dest" "sync/aegis-*" "Preview does not create sync branch in destination checkout"
    ...
    assert_contains "$bootstrap_output" "Mode:     BOOTSTRAP (creating plugins/aegis/ when absent)" "Bootstrap preview describes directory creation"
    ...
    assert_path_absent "$bootstrap_dest/plugins/aegis" "Bootstrap preview does not create destination plugin directory"
    ...
    assert_branch_absent "$bootstrap_dest" "bootstrap/aegis-*" "Bootstrap preview does not create bootstrap branch in destination checkout"

    echo ""
    echo "Apply assertions..."
    assert_equals "$dirty_apply_status" "1" "Dirty local apply exits with failure"
    assert_contains "$dirty_apply_output" "ERROR: local checkout has uncommitted changes under 'plugins/aegis'" "Dirty local apply reports protected destination path"
    ...
    assert_branch_absent "$dirty_apply_dest" "sync/aegis-*" "Dirty local apply does not create sync branch in destination checkout"
...
Locally modified fixture content." "Dirty local apply preserves tracked working-tree file content"
    ...
    assert_branch_absent "$noop_apply_dest" "sync/aegis-*" "Clean no-op local apply does not create sync branch in destination checkout"
