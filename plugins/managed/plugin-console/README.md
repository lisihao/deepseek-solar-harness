...
- **Solar Desktop startup**: startup never contacts the npm registry. Version checks run only after the user selects **Check for updates**, so an unavailable registry cannot delay or freeze the Desktop window.
