...
- **每回合实时记录**：快照内固定提示行要求模型**在最终回复消息中先输出完整回复文本、再在文本之后附带工具调用**——每轮收尾向项目日志/每日日志**各写入 1 条**本回合进展，并**按重要性判断**是否向项目关键记忆（`key`）**提交建议**（用户确认后写入并注入，见「分层记忆」表）。模型漏写的已启用日志轨由 `session/event` 的持久 `turn/end` 宿主闭环补写最终回复摘要，闭环轮次、来源和结果持久化到 `turn-state.json`；时间戳、每日日志的**项目标签**与日志的 **git 分支 tag**（`[git 分支名]`）均由程序自动标注；可在「Memory Evolve 设置」Tab 的「配置」分别关闭「每回合写入项目日志 / 每日日志 / 检查项目关键记忆」；
...
- **缓存友好**：记忆快照走 user-role 尾部消息注入（变更检测），system prompt 与历史前缀保持稳定。**只注入低频变化的轨**（用户档案/全局事实/**项目关键记忆**）——项目日志与每日日志随每回合主动写入或宿主补写而变化，若注入会导致每轮追加新的上下文快照、前缀缓存命中率下降，因此它们**默认不注入**，改为按需读取 + 快照中一行**固定提示**要求模型每轮收尾调用 memory 工具写入（提示文本固定不变，不产生新快照）。项目关键记忆（`key`）只在出现长期事实时才写入，低频稳定，与全局轨一样用**实时读取 + 变更检测**注入——写入后下一轮即出现在模型上下文中。
...
| 项目日志（`project`） | 当前项目的约定与进展 | **不注入**，按需读取（`memory` 工具，按 cwd 隔离） | 模型每回合主动写入；漏写时宿主补写（无需确认） |
| 每日日志（`daily`） | 今天做了什么 | **不注入**，按需读取（`memory` 工具） | 模型每回合主动写入；漏写时宿主补写（无需确认） |
...
- **触发**：插件从 `session/event` 的持久 `turn/end` 事实统计每个会话的用户回合数（仅 message 回合；子代理会话不计数），并持久化到 `turn-state.json`；达到 `reviewInterval` 后，一次审查被标记为**到期**；
- **执行**：**到期时快照末尾会直接出现醒目警告**（「记忆审查已到期」），模型收尾必须执行审查并 complete；无警告时无需轮询。`memory_review_status` 仍可手动查询到期状态和最近闭环收据。到期时模型在自己的回合内**静默执行审查**（工具操作，不写进最终回复）：
...
- **职责边界**：project/daily 由主会话主动写入并由宿主闭环补齐，key 按重要性判断写入（见「分层记忆」表），审查只处理全局轨建议与技能；
- **依赖**：审查由**提示词驱动**，依赖模型的指令遵循能力——弱遵循模型可能看到到期警告仍不执行（见「已知局限」）；日志轨闭环不依赖该能力。

审查产出（建议与技能）在会话列表中不可见（没有子代理），但建议队列与技能库可在会话页记忆 Tab 查看/确认。

### 每回合收尾执行

每个回合结束时，模型在**输出完整回复文本之后**附带工具调用：向 project/daily 各写入 1 条本回合进展（一次调用、`entries` 数组同时带两个轨），出现重要项目事实（长期约定/决策/架构/踩坑）时另向 key 写入 1 条。到期警告出现时静默执行审查（见上）；模型没有成功写入的日志轨由宿主补齐，重复 `turn/end` 投递和应用重启不会重复写：
...
用户会话进行中（每回合）
  ...
  ├─ 宿主闭环：核对 memory 工具结果；缺失的 project/daily 轨补写最终回复摘要并记录收据
  └─ 到期警告出现 → 回合内静默审查（不写进回复）：
