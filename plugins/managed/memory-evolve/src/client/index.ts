...
export const zh = {
  ...
  'panel.config.perTurnProjectWrites.hint': '开启后，每个已完成的用户回合都必须形成项目记忆：模型未主动写入时由宿主补写，并记录可检查的闭环回执；关闭后仅按需读取。',
  'panel.config.perTurnDailyWrites': '每回合写入每日日志',
  'panel.config.perTurnDailyWrites.hint': '开启后，每个已完成的用户回合都必须形成每日日志：模型未主动写入时由宿主补写，并记录可检查的闭环回执；关闭后仅按需读取。',
...
export const en: Record<MemoryEvolveKey, string> = {
  ...
  'panel.config.perTurnProjectWrites.hint': 'When enabled, every completed user turn must produce a project-memory entry. The host writes a fallback when the model does not and records an inspectable closure receipt. When disabled, project memory is read on demand only.',
  'panel.config.perTurnDailyWrites': 'Per-turn daily writes',
  'panel.config.perTurnDailyWrites.hint': 'When enabled, every completed user turn must produce a daily-log entry. The host writes a fallback when the model does not and records an inspectable closure receipt. When disabled, the daily log is read on demand only.',
