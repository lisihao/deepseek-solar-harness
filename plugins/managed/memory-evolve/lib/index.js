...
export function renderSnapshot(config, store, agent, counter, sessionTitleService = null) {
  ...
  if (memoryEntries.length > 0) {
    ...
    parts.push(`## 长期记忆（所有项目、会话都必须遵循）\n${memoryEntries.map((entry) => `- ${stripEntryId(entry)}`).join('\n')}`)
  }
  if (userEntries.length > 0) {
    parts.push(`## 用户档案\n${userEntries.map((entry) => `- ${stripEntryId(entry)}`).join('\n')}`)
  }
  ...
  if (keyEntries.length > 0) {
    ...
    // 展示剥离：身份证 [id:…] 不注入上下文（内部机制，施工图 §4.7）
    parts.push(`${head}\n${keyEntries.map((entry) => `- ${stripEntryId(entry)}`).join('\n')}`)
  }
...
function outcomeOnly(result) {
  if (result && typeof result === 'object') {
    ...
    if (Array.isArray(matches)) rest.matches = matches.map((entry) => stripEntryId(entry))
    return rest
  }
  return result
}
          ...
          case 'list': {
            ...
            if (args.archived === true) {
              ...
              result = {
                ...
                target,
                entries: entries.map((entry) => stripEntryId(entry)), // 展示剥离身份证（§4.7）
                chars: entries.map((entry) => stripEntryId(entry)).join('\n').length,
              }
              break
            }
            const stats = {}
            ...
            result = {
              ...
              entries: entries.map((entry) => stripEntryId(entry)),
              chars: entries.map((entry) => stripEntryId(entry)).join('\n').length,
              total,
              earliest,
              latest,
