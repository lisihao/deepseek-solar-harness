...
describe('registry peer contract (Solar rc.6 plus upstream rc.2, no link farm)', () => {
  ...
  it('every @deepseek-ai/dsh-* peer accepts the Desktop and upstream runtime families', () => {
    for (const [name, range] of Object.entries(root.peerDependencies ?? {})) {
      if (name.startsWith('@deepseek-ai/dsh-')) {
        expect(range, name).toBe('^0.1.0-rc.6 || ^0.1.1-rc.2')
      }
    }
  })
